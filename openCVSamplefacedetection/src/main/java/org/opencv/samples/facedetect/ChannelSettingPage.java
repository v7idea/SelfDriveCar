...
public class ChannelSettingPage extends Activity implements View.OnClickListener
{
    public static final String TAG = ChannelSettingPage.class.getSimpleName();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Banner Header = (Banner) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_Banner_Header);
        Header.initV7rcStyle();
        Header.addButton("", R.mipmap.button_menu_back, this);
        Header.setTitleString(settingData.getName());

        SpecialImageView background = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_background);

        subTrimBackground = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_SubTrimBackground);
        offSet = ((RelativeLayout.LayoutParams)subTrimBackground.getLayoutParams()).leftMargin + (int)(30 * ViewScaling.getScaleValue());
        controlBarWidth = (float) (subTrimBackground.getLayoutParams().width);
//        controlBarWidth = (float) (774 * ViewScaling.getScaleValue());

        lowerValue = (V7TitleView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_V7TitleView_lowerValue);

        lowerLimitBar = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_lowerLimitBar);

        upperValue = (V7TitleView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_V7TitleView_upperValue);

        upperLimitBar = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_upperLimitBar);

        middleValue = (V7TitleView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_V7TitleView_middleValue);

        middleLimitBar = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_middleLimitBar);

        failSafeBar = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_failSafeBar);

        failSafeValue = (V7TitleView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_V7TitleView_failSafeValue);

        ExpotentailImage = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_Expotentail);
        ExpotentailImage.setOnClickListener(onPressExpotentail);

        reverseSwitch = (SpecialImageView) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_SpecialImageView_ReverseSwitch);
        reverseSwitch.setOnClickListener(onPressSwitch);

        bottomMenu = (BottomMenuLayout) ViewScaling.findViewByIdAndScale(this, R.id.ChannelSettingPage_BottomMenuLayout_BottomMenu);
        bottomMenu.setting();
        bottomMenu.setVisibility(View.VISIBLE);
        bottomMenu.setOnPressCallBack(onPressCallBack);
        bottomMenu.cancelOnClickListenerByIndex(bottomMenu.getPageIndex());

        Channel channel1 = settingData.getChannel(0);
        Channel channel2 = settingData.getChannel(1);
        ...
        bleController.setChannel1((int)(channel1.getMiddleValue()));
        bleController.setChannel2((int)(channel2.getMiddleValue()));
        bleController.startCommand();

        Log.e(TAG, "channel1 value: "+bleController.getChannel1());
        Log.e(TAG, "channel2 value: "+bleController.getChannel2());
    ...
    private int changeImageResourceIDByChannel()
    {
        ...
        simpleDatabase.setValueByKey(ExpotentailChannelName, String.valueOf(expotentialValue[expotentailIndex]));

//        dataBase.EditUpdate(settingData.getId()
...
//        DebugLog.e(TAG, "marginLeft: "+marginLeft);
    ...
    {
        float processEvent = floatEventX;

        return  (multiple * processEvent) + GRADUATION_START;
    }
    ...
    private void changeMarginLeftByView(View view, int intMarginLeft)
    {
        ...
        if(view.getId() == R.id.ChannelSettingPage_SpecialImageView_lowerLimitBar)
        {
            viewParams.leftMargin = intMarginLeft - (viewParams.width / 2);
        }
        else if(view.getId() == R.id.ChannelSettingPage_SpecialImageView_upperLimitBar)
        {
            viewParams.leftMargin = intMarginLeft - (viewParams.width / 2);
        }
        else if(view.getId() == R.id.ChannelSettingPage_SpecialImageView_middleLimitBar)
        {
            viewParams.leftMargin = intMarginLeft - (viewParams.width / 2);
            //設定要被移動的ImageView
        }
        else if(view.getId() == R.id.ChannelSettingPage_SpecialImageView_failSafeBar)
        {
            //過濾FailSaveBar移動的範圍
    ...
    private View.OnClickListener onPressSwitch = new View.OnClickListener() {
        ...
        public void onClick(View v)
        {
            ...
            if(isReverse)
            {
                ...
                simpleDatabase.setValueByKey(fieldName, "Yes");
//                dataBase.EditUpdate(settingData.getId(), fieldName, "Yes");
            }
            else
            {
                settingData.getServoReverseItem().setChannelData(channelIndex, "NO");
                simpleDatabase.setValueByKey(fieldName, "NO");
//                dataBase.EditUpdate(settingData.getId(), fieldName, "NO");
            }
        }
    };
...
//            private float valueMaxmun = 0f;
            ...
            {
                if(v.getId() == R.id.ChannelSettingPage_SpecialImageView_lowerLimitBar)
                {
                    //設定要被移動的ImageView
                ...
                else if(v.getId() == R.id.ChannelSettingPage_SpecialImageView_upperLimitBar)
                {
                    //設定要被移動的ImageView
                ...
                else if(v.getId() == R.id.ChannelSettingPage_SpecialImageView_middleLimitBar)
                {
                    //設定要被移動的ImageView
                ...
                else if(v.getId() == R.id.ChannelSettingPage_SpecialImageView_failSafeBar)
                {
                    //設定要被移動的ImageView
...
//                DebugLog.d(TAG, "valueMaxmun: "+valueMaxmun);
    ...
    public boolean onTouchEvent(MotionEvent event)
    {
        ...
        DebugLog.e(TAG, "moveScale: "+moveScale);
...
//                DebugLog.e(TAG, "move offSet: "+offSet);
...
//                    }
            ...
            else if(event.getAction() == MotionEvent.ACTION_UP)
            {
                ...
                if(valueCurrent <= valueMaxmun && valueCurrent >= valueMinimum)
                {
                    ...
                    {
                        case R.id.ChannelSettingPage_SpecialImageView_lowerLimitBar:

                            settingData.getStrimConfigItem(currentChannelIndex).setLower(strSaveValue);
                            simpleDatabase.setValueByKey(Constants.getStrimConfigLowerKeyName(currentChannelIndex+1), strSaveValue);

//                            dataBase.EditUpdate(settingDataInDBID, "StrimConfigCh"+(currentChannelIndex+1)+"Lower"
//                                    , settingData.getStrimConfigItem(currentChannelIndex).getLower());
//                            DebugLog.d("SQL","tView.getId():   "+tempControlBar.getId());
                            break;

                        case R.id.ChannelSettingPage_SpecialImageView_middleLimitBar:

                            settingData.getStrimConfigItem(currentChannelIndex).setMiddle(strSaveValue);
                            simpleDatabase.setValueByKey(Constants.getStrimConfigMiddleKeyName(currentChannelIndex+1), strSaveValue);
//                            dataBase.EditUpdate(settingDataInDBID, "StrimConfigCh"+(currentChannelIndex+1)+"Middle"
//                                    , settingData.getStrimConfigItem(currentChannelIndex).getMiddle());
//                            DebugLog.d("SQL","tView.getId():   "+tempControlBar.getId());
                            break;

                        case R.id.ChannelSettingPage_SpecialImageView_upperLimitBar:

                            settingData.getStrimConfigItem(currentChannelIndex).setUpper(strSaveValue);
                            simpleDatabase.setValueByKey(Constants.getStrimConfigUpperKeyName(currentChannelIndex+1), strSaveValue);
//                            dataBase.EditUpdate(settingDataInDBID, "StrimConfigCh"+(currentChannelIndex+1)+"Upper"
//                                    , settingData.getStrimConfigItem(currentChannelIndex).getUpper());
//                            DebugLog.d("SQL","tView.getId():   "+tempControlBar.getId());
                            break;

                        case R.id.ChannelSettingPage_SpecialImageView_failSafeBar:

                            settingData.getStrimConfigItem(currentChannelIndex).setFailSafe(strSaveValue);
                            simpleDatabase.setValueByKey(Constants.getStrimConfigFailSafeKeyName(currentChannelIndex+1), strSaveValue);
//                            dataBase.EditUpdate(settingDataInDBID, "StrimConfigCh"+(currentChannelIndex+1)+"FailSafe"
//                                    , settingData.getStrimConfigItem(currentChannelIndex).getFailSafe());
//                            DebugLog.d("SQL","tView.getId():   "+tempControlBar.getId());
                            break;
                    }

                    sendDataRunnable.setValue(strSaveValue);
